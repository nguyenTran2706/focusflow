  ...
  return (
    <div className="flex min-h-screen">
      ...
      <main className="flex-1 ml-[var(--spacing-sidebar)] flex flex-col min-h-screen">
        ...
        {/* Toast */}
        {toast && (
          <div className={`mx-6 mt-4 px-4 py-3 rounded-lg text-[0.85rem] font-medium animate-fade-in ${
        ...
        <div className="flex-1 p-6 max-w-[720px]">
          ...
          <div className="flex items-center gap-5 mb-8 animate-fade-in">
            <div className="w-[72px] h-[72px] rounded-full overflow-hidden border-2 border-border-subtle shrink-0">
              {clerkUser?.imageUrl ? (
            ...
            <div>
              <h2 className="text-[1.25rem] font-semibold text-text-primary">{dbUser?.name ?? 'Loading...'}</h2>
              ...
              <div className="flex items-center gap-2 mt-1.5">
                ...
                {dbUser?.role === 'ADMIN' && (
                  <span className="text-[0.7rem] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full bg-warning/10 text-warning">Admin</span>
                )}
              </div>
            </div>
          </div>
          ...
          <section className="bg-bg-card border border-border-subtle rounded-xl p-6 mb-6 animate-fade-in">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="flex flex-col gap-1.5">
                ...
                <input className={inputCls(editing)} value={name} onChange={(e) => setName(e.target.value)} readOnly={!editing} placeholder="Your name" />
              </div>
              <div className="flex flex-col gap-1.5">
                <label className="text-[0.75rem] font-semibold text-text-muted uppercase tracking-wider">Email</label>
                <input className={inputCls(false)} value={dbUser?.email ?? ''} readOnly title="Managed by Clerk" />
              </div>
              <div className="flex flex-col gap-1.5">
                <label className="text-[0.75rem] font-semibold text-text-muted uppercase tracking-wider">Phone</label>
                <input className={inputCls(editing)} value={phone} onChange={(e) => setPhone(e.target.value)} readOnly={!editing} placeholder="Phone number" />
              </div>
              <div className="flex flex-col gap-1.5">
                <label className="text-[0.75rem] font-semibold text-text-muted uppercase tracking-wider">Account Created</label>
                <input className={inputCls(false)} value={dbUser?.createdAt ? new Date(dbUser.createdAt).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' }) : ''} readOnly />
              </div>
            </div>
          </section>
          ...
          <section className="bg-bg-card border border-border-subtle rounded-xl p-6 mb-6 animate-fade-in" style={{ animationDelay: '60ms' }}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="flex flex-col gap-1.5 md:col-span-2">
                <label className="text-[0.75rem] font-semibold text-text-muted uppercase tracking-wider">Street Address</label>
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-[0.75rem] font-semibold text-text-muted uppercase tracking-wider">City</label>
                <input className={inputCls(editing)} value={city} onChange={(e) => setCity(e.target.value)} readOnly={!editing} placeholder="City" />
              </div>
              <div className="flex flex-col gap-1.5">
                <label className="text-[0.75rem] font-semibold text-text-muted uppercase tracking-wider">State / Province</label>
                <input className={inputCls(editing)} value={state} onChange={(e) => setState(e.target.value)} readOnly={!editing} placeholder="State" />
              </div>
              <div className="flex flex-col gap-1.5">
                <label className="text-[0.75rem] font-semibold text-text-muted uppercase tracking-wider">Postal Code</label>
                <input className={inputCls(editing)} value={postal} onChange={(e) => setPostal(e.target.value)} readOnly={!editing} placeholder="12345" />
              </div>
              <div className="flex flex-col gap-1.5">
                <label className="text-[0.75rem] font-semibold text-text-muted uppercase tracking-wider">Country</label>
                <input className={inputCls(editing)} value={country} onChange={(e) => setCountry(e.target.value)} readOnly={!editing} placeholder="Country" />
              </div>
            </div>
          </section>
          ...
          <section className="bg-bg-card border border-border-subtle rounded-xl p-6 animate-fade-in" style={{ animationDelay: '120ms' }}>
            ...
            <div className="flex items-center justify-between">
              <div>
                <p className="text-text-primary font-medium">{subInfo?.label} Plan</p>
                <p className="text-text-muted text-[0.8rem] mt-1">
                  {dbUser?.subscription === 'FREE' ? 'Basic access with limited features' :
                   dbUser?.subscription === 'PRO' ? 'Full feature access with priority support' :
                   'Everything in Pro plus dedicated support'}
                </p>
              </div>
              <a href="/pricing" className="px-4 py-[7px] rounded-md text-[0.85rem] font-medium border border-accent text-accent hover:bg-accent hover:text-white transition-colors">
                {dbUser?.subscription === 'FREE' ? 'Upgrade' : 'Manage'}
              </a>
            </div>
          </section>
        </div>
