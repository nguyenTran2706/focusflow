...
export function DiagramListPage() {
  ...
  const { data: diagrams, isLoading } = useDiagrams(boardId);
          ...
          <div className="grid grid-cols-[repeat(auto-fill,minmax(260px,1fr))] gap-4 animate-fade-in">
            {diagrams.map((dg, i) => (
              <div
                ...
                onClick={() => navigate(`/boards/${boardId}/diagrams/${dg.id}`)}
              >
                <div className="flex items-center justify-between mb-3">
                  <div className="w-[34px] h-[34px] rounded-md flex items-center justify-center font-semibold text-[0.95rem] text-white bg-[hsl(200,60%,50%)]">
                    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round">
                      <circle cx="6" cy="6" r="3" /><circle cx="18" cy="18" r="3" />
                      <path d="M8.5 8.5l7 7" />
                    </svg>
                  </div>
                  {confirmDeleteId === dg.id ? (
                    <div className="flex items-center gap-1" onClick={(e) => e.stopPropagation()}>
                      <button
                        className="px-2 py-1 rounded text-[0.7rem] font-medium bg-danger text-white"
                        onClick={() => { deleteMut.mutate(dg.id); setConfirmDeleteId(null); }}
                      >
                        Delete
                      </button>
                      <button
