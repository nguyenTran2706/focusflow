export type ShareResourceType = 'whiteboard' | 'board' | 'diagram';
...
export function buildShareInvitationHtml(data: ShareInvitationEmailData): string {
  const roleLabel = data.role === 'EDITOR' ? 'edit' : 'view';
  const resourceLabel = data.resourceType;
  return `
<!DOCTYPE html>
<html lang="en">
      ...
      <table width="560" cellpadding="0" cellspacing="0" style="background:#1e1e22;border-radius:16px;border:1px solid rgba(255,255,255,0.08);overflow:hidden;">
        <tr>
          <td style="padding:32px 32px 24px;text-align:center;background:linear-gradient(135deg,rgba(99,102,241,0.15),rgba(168,85,247,0.10));">
            ...
            <h1 style="margin:0;font-size:22px;font-weight:700;color:#ffffff;">You've been invited to a ${resourceLabel}</h1>
            <p style="margin:12px 0 0;font-size:14px;color:#9394a0;line-height:1.5;">
              ...
              <strong style="color:#ffffff;">${escapeHtml(data.resourceName)}</strong> on FocusFlow.
            </p>
          </td>
        </tr>
        <tr>
          <td style="padding:24px 32px 32px;text-align:center;">
            ...
            <p style="margin:16px 0 0;font-size:11px;color:#6b6c78;">
              No FocusFlow account yet? You'll be able to sign up at this link.
            </p>
          </td>
        </tr>
      </table>
